...
<div id="fb-root"></div>
<script async defer crossorigin="anonymous" src="https://connect.facebook.net/id_ID/sdk.js#xfbml=1&version=v3.2&appId=<?= config_item('fbappid') ?>&autoLogAppEvents=1"></script>
        ...
        <div class="container<?= config_item('fluid') ? '-fluid' : '' ?>">
            <div class="row align-items-center <?php if(empty($_GET['cari']) && (in_array($this->uri->segment(1), ['', 'index'])) && (in_array($this->uri->segment(2), ['', 'index'])) && (config_item('menu') == false)) : ?>mb-5 mt-0<?php endif; ?>">
                ...
                <div class="col-12 col-sm text-center mt-3 text-sm-left text-white">
                    ...
                    <p class="text-hide-xs alamatlaikit"><?= $desa['alamat_kantor']?><br><?= ucwords($this->setting->sebutan_kecamatan." ".$desa['nama_kecamatan'])?> <?= ucwords($this->setting->sebutan_kabupaten." ".$desa['nama_kabupaten'])?> Provinsi <?= $desa['nama_propinsi']?><br>Kode Pos <?= $desa['kode_pos']?></p>
                    <p class="small">
                ...
                <div class="text-center text-sm-right col-12 col-sm-auto mb-3">
                    <a class="btn btn-sm btn-success success-gradient ml-2 anim-wa" href="https://api.whatsapp.com/send?l=id&text=<?= "https://".$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI]?>%0A%0A" onclick='window.open(this.href,"popupwindow","status=0,height=500,width=500,resizable=0,top=50,left=100");return false;' rel='noopener noreferrer' target='_blank' title='WhatsApp'><i class="material-icons">send</i> WhatsApp</a>
